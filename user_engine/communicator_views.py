...
def stream_users_by_topics_view(request):
    if request.method == "POST":
        try:
            ...
            if 'topic_id' in data:
                stream_data = communicator_object.stream_user_objects_by_topics(data['topic_id'])
    ...
    if request.method == "POST":
        try:
            ...
            if 'topic_id' in data and 'user_id' in data:
                stream_data = communicator_object.update_user_topics(data['topic_id'], data['user_id'])
                return make_http_response(stream_data)
        except Exception as E:
            return make_http_response({"status": 200,
                                     "message": "Cannot unload data."})
