...
class LeafInteraction(models.Model):
    leaf = models.ForeignKey(Leaf,  related_name="interacted_leaf", on_delete=models.CASCADE)
    interacted_by = models.ForeignKey(UserProfile, related_name="related_user", on_delete=models.CASCADE)
...
class UserLeafPreferences(models.Model):
    ...
    user_object = models.ForeignKey(UserProfile, on_delete=models.CASCADE,related_name='user_obj')
    created_at = models.DateTimeField(auto_now_add=True)
...
class UserTopicRelations(models.Model):
    ...
    user = models.ForeignKey(UserProfile, on_delete=models.CASCADE, related_name='topic_relation_user')
